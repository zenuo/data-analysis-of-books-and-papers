...
object BookPaperRelationshipByFieldAndCLC {
  def main(args: Array[String]): Unit = {
    //判断文件参数是否正确
    if (args.length != 5) {
      println("-------------------------------ERROR-------------------------------")
      println("Valid program arguments:")
      println("PATH_TO_book_id_callId.txt PATH_TO_cls_no_name.txt PATH_TO_paper_id_paperId.txt PATH_TO_paper_paperId_field.txt PATH_TO_resultFile")
    ...
    val spark = SparkSession
      .builder()
      .appName("BookPaperRelationshipByFieldAndCLC")
      .getOrCreate()
    ...
    //paper_paperId_field
    val paperPaperIdFieldTupleList = spark.sparkContext.textFile(paperInfoPath02).map(line => {
      val tokens = line.split(delimiter01).map(_.trim)
      //类似(TGZG200701002,铁路)
      tokens(0) -> tokens(1)
    })
    //join两个MapPartitionsRDD
    val paperFieldIdTupleList = paperPaperIdFieldTupleList.join(paperIdPaperIDTupleList).map(tuple => {
      //类似(铁路,112)
      tuple._2
    })
    //paperFieldIdTupleList.count(): 570036
    //paperFieldIdTupleList.first(): (外国语言文字,300221)

    //分析联系
    //join两个MapPartitionsRDD
    val paperBookRelationshipList = paperFieldIdTupleList.join(bookCLCNameIdTupleList).map(tuple => {
      //类似(100,122)
      tuple._2
    })
