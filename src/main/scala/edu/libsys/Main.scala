...
object Main {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val indOfppi: RDD[(Int, Int)] = sc.textFile(sourcePath + "indOfppi.txt").map(stringToTuple)
    val indOfbpa: RDD[(Int, Int)] = sc.textFile(sourcePath + "indOfbpa.txt").map(stringToTuple)
    val indOfbcnpf: RDD[(Int, Int)] = sc.textFile(sourcePath + "indOfbcnpf.txt").map(stringToTuple)
    val indOfbcnpi: RDD[(Int, Int)] = sc.textFile(sourcePath + "indOfbcnpi.txt").map(stringToTuple)
    ...
    weight.filter(_._1 < Conf.paperIdOffset)
      .map(weightToString)
      .saveAsTextFile(resultPath + "books")

    weight.filter(_._1 > Conf.paperIdOffset)
      .map(weightToString)
      .saveAsTextFile(resultPath + "papers")
